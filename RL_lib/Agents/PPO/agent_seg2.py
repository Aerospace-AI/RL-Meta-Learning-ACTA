...
class Agent(object):
    ...
    def segment_trajectory(self, trajectory):
        ...
        for i in range(num_seg):
            seg = {}
            for k in keys:
                seg[k] = trajectory[k][i*self.seg_len:i*self.seg_len+self.seg_len]
            seg_trajectory.append(seg)
        return seg_trajectory
    ...
    def add_disc_sum_rew(self,trajectories, gamma1, gamma2):
        ...
        for trajectory in trajectories:
            ...
            seg_trajectory = self.segment_trajectory(trajectory)
            all_targs1 = []
            all_targs2 = []

            for i in range(len(seg_trajectory)):
                seg = seg_trajectory[i]
                done = seg['dones'][-1]
                rewards1 = seg['rewards1']
                rewards2 = seg['rewards2']
                if gamma1 < 0.999:  # don't scale for gamma ~= 1
                    rewards1 = rewards1 * (1 - gamma1)
                else:
                    rewards1 = rewards1 * (1-0.999)
                if gamma2 < 0.999:  # don't scale for gamma ~= 1
                    rewards2 = rewards2 * (1 - gamma2)
                else:
                    rewards2 = rewards2 * (1-0.999)

                if not done:
                    vpred = seg_trajectory[i+1]['vpreds'][0] 
                    rewards1 = np.hstack((rewards1, vpred ))
                    rewards2 = np.hstack((rewards2, vpred )) 
                ...
                all_targs1.append(disc_sum_rew1)
                all_targs2.append(disc_sum_rew2)
            
            trajectory['disc_sum_rew'] = np.concatenate(all_targs1) + np.concatenate(all_targs2)
