...
class Arch(object):
    def __init__(self,   dyna, zero_model_errors=False, use_model=False, get_lander_state=None):
    ...
    def run_episode(self, env, policy, val_func, model, recurrent_steps, use_timestep, add_padding=True):
        ...
        obs = env.reset()
        muxed_observes , observes, actions,  rewards1, rewards2, nobserves,   model_states, model_errors, model_predicts,  model_vpreds, policy_states, vf_states, vpreds   =     [], [], [], [], [], [], [], [], [], [], [], [], []
        traj = {}
        done = False
        step = 0.0
        ...
        while not done:
            ...
            model_vpreds.append(model_vpred)
            ...
            vpred, vf_state = val_func.predict(obs_muxed, vf_state)
            vpreds.append(vpred)
        ...
        traj['model_vpreds'] = np.array(model_vpreds, dtype=np.float64) 
    ...
    def update(self,policy,val_func,model,rollouts, logger):
        ...
        sim_rollouts = self.dyna.simulate(model, policy, val_func)
        policy.update(sim_rollouts, logger)
