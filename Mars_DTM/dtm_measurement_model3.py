...
class DTM_measurement_model(object):
    ...
    def __init__(self,dtm,vertical_resolution=1.0,check_vertical_errors=False,max_reading=10000,print_missing_beams=False,print_stats_every=100000):
        self.dtm = dtm
    ...
    def get_altimeter_reading(self, altimeter_dvec, altimeter_location, altimeter_velocity, debug=False):
        ...
        if bad1 or bad2:
            ...
            self.miss_cnt += 1
            default_intersection = intersections[self.middle]  
            default_v_doppler = self.get_vc(altimeter_dvec,altimeter_velocity)
            default_altitude = np.linalg.norm(default_intersection - altimeter_location) 
            default_altitude = np.clip(default_altitude,0,self.max_reading)
            return default_altitude, default_v_doppler , default_intersection 
        ...
        altitude = np.clip(altitude, 0, self.max_reading)
