...
class Altimeter(object):
    ...
    def get_inertial_dvecs(self, dvecs_b, position, velocity, attitude):
        C = self.attitude_parameterization.get_body_to_inertial_DCM(attitude)
        dvecs_i = C.dot(dvecs_b.T).T
        #dvec_i = C.dot(self.refdvec_b)
        #C = attu.DCM(self.refdvec_b, dvec_i)
        #dvecs_i = C.dot(dvecs_b.T).T
        return dvecs_i

    def get_reading(self, position, velocity, attitude):
        dtm_position = position + self.target_position

        dvecs_b = self.get_body_frame_attitude()
        dvecs_i = self.get_inertial_dvecs(dvecs_b, position, velocity, attitude)
        altitudes = [] 
        cvs = []
        #print('FOO: ', dtm_position, velocity)
        for dv in dvecs_i:
            #print('DV ',dv ) 
            a, v, foo = self.measurement_model.get_altimeter_reading(dv, dtm_position, velocity)
            #print('intercept: ',foo)
