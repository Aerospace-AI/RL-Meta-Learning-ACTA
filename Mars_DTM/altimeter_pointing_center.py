...
class Altimeter(object):
    def __init__(self, measurement_model, target_position, pointing_target=None, dphi=0.0, theta=np.pi/8, nbeams=4,  attitude_parameterization=attu.Quaternion_attitude()):
    ...
    def get_body_frame_attitude(self):
        ...
        for i in range(self.phis.shape[0]):
            dvecs_b.append(-np.asarray([np.sin(self.theta)*np.cos(self.phis[i]),
                                     ...
                                     np.cos(self.theta)]))
        dvecs_b.append(np.asarray([0.,0.,-1.]))        
