...
class Lander_model(object):
    ...
    def __init__(self, altimeter=None, use_trajectory_list=False, divert = (0.0,  0.0, 0.0), sensor_bias_range=(0.0,0.0), sensor_sd=0.0, 
        ...
        self.traj = {}
        self.state_keys = ['position','velocity','thrust','mass','t_go', 'disturbance']
    ...
    def get_state_agent2(self,t):
        position = self.add_noise(self.state['position'])
        velocity = self.add_noise(self.state['velocity'])
        error, t_go = self.track_func(position, velocity)
    ...
    def get_state_agent5(self,t):
        ...
        velocity = self.add_noise(self.state['velocity'])
        error, t_go = self.track_func(position, velocity)
        state = np.hstack((error, t_go, position[2]))
        self.state['t_go'] = t_go
        return state
    ...
    def get_landing_state(self,t):
        ...
        landing_state['disturbance'] = self.state['disturbance'] 
    ...
    def get_engagement_keys(self):
        keys = ['t', 'norm_rf', 'norm_vf', 'position', 'velocity', 'thrust', 'disturbance','mass', 'fuel', 'vc', 'range', 'los', 'dlos', 'zem','reward','fuel_reward','landing_reward','tracking_reward','glideslope_reward', 'landing_margin', 'glideslope', 'sc_margin', 'glideslope_penalty','sc_penalty','sc_reward','range_reward','v_ratio', 't_go', 'value']
        return keys
