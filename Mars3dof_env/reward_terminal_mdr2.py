...
class Reward(object):
    ...
    def __init__(self, reward_scale=1.0, ts_coeff=-0.0,  fuel_coeff=-0.05, term_fuel_coeff=0.02,
                 landing_rlimit=5.0, landing_vlimit=2.0, landing_gslimit=5.0, landing_coeff=10.0,
        ...
        print('Reward MDR Test: ',self.term_fuel_coeff, self.term_fuel_coeff * 250)
    ...
    def get(self, lander,  action, done, steps, shape_constraint, glideslope_constraint):
        ...
        if done:
            ...
            if pos[2] < 0 and np.linalg.norm(pos) < self.landing_rlimit and np.linalg.norm(vel) < self.landing_vlimit and gs > self.landing_gslimit:
                mass_delta = np.maximum(0.0 , lander.state['mass'] - 1500)
                fuel_bonus =  mass_delta * self.term_fuel_coeff    
                r_landing = self.landing_coeff + fuel_bonus
        ...
        reward1 = (sc_penalty + gs_penalty + r_gs + r_sc + r_tracking +  r_fuel + self.ts_coeff) * self.reward_scale
        reward2 = r_landing
