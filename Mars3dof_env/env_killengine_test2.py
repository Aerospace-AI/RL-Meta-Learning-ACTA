...
class Env(object):
    def __init__(self, lander, dynamics, logger,  
        ...
        self.episode = 0
        self.p_kill = p_kill
        self.kill_lateral_scale = kill_lateral_scale
        self.kill_vertical_scale = kill_vertical_scale
        self.engine = None
    ...
    def reset(self): 
        ...
        self.engine = np.random.randint(low=0,high=1)
    ...
    def step(self,action):
        ...
        if self.kill:
            action = envu.kill_engine_test(action, self.engine, self.kill_lateral_scale, self.kill_vertical_scale)
        #print('AFTER: ',action)
