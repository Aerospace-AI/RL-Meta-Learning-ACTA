...
class Lander_model(object):
    ...
    def __init__(self, altimeter=None, use_trajectory_list=False, divert = (0.0,  0.0, 0.0),
                 sensor_pos_bias_range=(0.0,0.0), sensor_vel_bias_range=(0.0,0.0), consistent_bias=False,
        ...
        self.sensor_pos_bias_range = sensor_pos_bias_range
        self.sensor_vel_bias_range = sensor_vel_bias_range
        self.consistent_bias = consistent_bias
    ...
    def clear_trajectory(self):
        for k in self.get_engagement_keys():
            self.trajectory[k] = []
        self.sensor_pos_bias = np.random.uniform(self.sensor_pos_bias_range[0], self.sensor_pos_bias_range[1],size=3)
        self.sensor_vel_bias = np.random.uniform(self.sensor_vel_bias_range[0], self.sensor_vel_bias_range[1],size=3)
    ...
    def get_state_agent_eo2(self,t):
        landed = self.state['position'][2] < 0
        error, t_go = self.track_func(self.state['position'], self.state['velocity'])
        self.state['t_go'] = t_go
        state = np.hstack((error, landed))
        return state
    ...
    def get_state_agent5(self,t):
        #print('before: ', self.state['position'], self.state['velocity'])
        pos,vel = self.add_noise()
        #print('after: ', pos,vel)
        error, t_go = self.track_func(pos, vel)
        state = np.hstack((error, t_go, pos[2]))
        self.state['t_go'] = t_go
        return state
    ...
    def add_noise(self):
        pos = self.add_noise_to_obs(self.state['position'], self.sensor_pos_bias)
        if self.consistent_bias:
            vel = self.add_noise_to_obs(self.state['velocity'], self.sensor_pos_bias)
        else:
            vel = self.add_noise_to_obs(self.state['velocity'], self.sensor_vel_bias)

        return pos,vel

    def add_noise_to_obs(self, obs, obs_bias):
        #print('before: ',obs)
        bias = obs_bias * np.abs(obs)
        obs = obs + bias
        #print('after: ' , obs)
        return obs
