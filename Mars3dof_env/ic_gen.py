...
class Landing_icgen(object):
    ...
    def __init__(self, downrange = (500,1500 , -70, -10), crossrange = (-500,500 , -30,30),  altitude = (1900,2100,-90,-70) , scale=None, 
                 noise_u=np.zeros(3), noise_sd=np.zeros(3), min_mass=2000., max_mass=2000., min_g=(0.5, -0.2),  max_g=(-4, 0.2),  adjust_apf_v0=True): 
    ...
    def show(self):
        print('Landing_icgen:')
        print('    min_g                   : ',self.min_g)
        print('    max_g                   : ',self.max_g)
        print('    min_mass                : ',self.min_mass)
        print('    max_mass                : ',self.max_mass)
    ...
    def set_ic(self , lander, dynamics):
        ...
        dynamics.g[2] =  np.random.uniform(low=self.min_g[0], high=self.max_g[0])

        dynamics.g[0:2] = np.random.uniform(low=self.min_g[1], high=self.max_g[1], size=2)
        #print(dynamics.g)                                           
