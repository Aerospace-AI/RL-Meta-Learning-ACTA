...
class Altimeter(object):
    def __init__(self, measurement_model, target_position,  pointing_position, dphi=0.0, theta=np.pi/8, nbeams=4,  attitude_parameterization=attu.Quaternion_attitude()):
        ...
        self.miss_cnt = 0
        self.cnt = 0
        print('PNT target: ', self.pointing_position) 
    ...
    def get_reading(self, position, velocity):
        ...
        pnt_position = position + self.pointing_position

        dvecs_i = self.get_inertial_dvecs(self.dvecs_b, pnt_position, velocity)
        altitudes = [] 
        cvs = []
        vtests = []
