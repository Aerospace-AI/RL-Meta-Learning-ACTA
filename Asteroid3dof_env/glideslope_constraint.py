...
class Glideslope_constraint(object):
    def __init__(self, delta=3, debug=False, gs_coeff=-1.0, gs_margin=0.1, gs_limit= 0.1, gs_tau=0.05, gs_penalty=-50.0, terminate_on_violation=True):
        self.delta = delta
        self.debug = debug
        ...
        print('Glideslope Constraint: delta = ',delta)
    ...
    def calculate(self, state):
        ...
        if indices.shape[0] > 0:        
            ...
            if self.debug:
                print('GS: ',gs, dx,dy,dz, state['velocity'])
            self.gs = gs
        else:
            gs = 100.
        self.gs = gs
