...
class Dynamics_model(object):
    ...
    def __init__(self, h=0.5, noise_u=np.zeros(3),noise_sd=np.zeros(3), w=np.zeros(3), g=np.zeros(3), srp=np.zeros(3), landing_target=np.asarray([250., 0., 0.]), w_o=2*np.pi/2000):
        self.h = h
